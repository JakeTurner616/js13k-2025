...
/** Register background effects (stars + moon) exactly once. */
  ...
  addB(((c,w,h,_t,cx)=>{
    ...
    for(let i=12;i--;){
      const a=H(i)*6.283,d=r*(.18+.68*H(i+1)),
            x=mx+Co(a)*d,y=my+S(a)*d;
      if((x-mx)**2+(y-my)**2>r*r*.83) continue;
      const cr=POS(r*(.02+.05*H(i+2))),k=.35+.65*H(i+3);
      c.fillStyle="rgba(40,45,60,.26)";
      c.beginPath();
      c.arc(x,y,cr,0,7);
      c.fill();
      c.strokeStyle="rgba(230,235,255,.10)";
      c.lineWidth=POS(cr*.34);
      c.beginPath();
      c.arc(x,y,POS(cr*k),0,7);
      c.stroke();
    }
  }) as Draw);
}
