...
import { addB, S, Co, H, POS, col, NEON, rgba } from "../u";

// ---------------- Stars ----------------
addB(((c,w,h,t,scroll)=>{
  c.fillStyle = col(0);                // #fff
  const cutoff=h*.35;
  for(let i=0;i<60;i++){
...
addB(((c,w,h,t,cx)=>{
  ...
  // halo (explicit RGBA; not palette-indexed to avoid range issues)
  let g = c.createRadialGradient(mx,my,0,mx,my, r*2.2);
  g.addColorStop(0, "rgba(210,225,255,.14)");
  g.addColorStop(1, "rgba(0,0,0,0)");
  c.fillStyle=g; const hr=r*2.2; c.fillRect(mx-hr,my-hr,hr*2,hr*2);

  // disc via palette 12/13/14
  ...
  for(let i=0;i<18;i++){
    ...
    c.fillStyle="rgba(40,45,60,.26)"; c.beginPath(); c.arc(cx0,cy0,cr,0,7); c.fill();
    c.strokeStyle="rgba(230,235,255,.10)"; c.lineWidth=POS(cr*.34);
    c.beginPath(); c.arc(cx0,cy0,POS(cr*k),0,7); c.stroke();
  }
...
addB(((c,w,h,t,cx)=>{
  for(let i=0;i<6;i++){
    ...
    g.addColorStop(0, rgba(NEON[i&1], a));
    g.addColorStop(1, "rgba(0,0,0,0)");
