...
const walls=(c:CanvasRenderingContext2D,x:number,y:number,fwR:number,side:number,fh:number,dep:number,L:number,R:number)=>{
  const tr:any=(c as any).getTransform?c.getTransform():{a:1,d:1}, ex=.5/tr.a, ey=.5/tr.d, fwSum=side+fwR;
...
const windows=(c:CanvasRenderingContext2D,x:number,y:number,v:BV,cL:number,cR:number,fwL:number,fwR:number,side:number,fh:number,dep:number)=>{
  ...
  const face=(cols:number,bx:number,by:number,fw:number,dm:number,base:string)=>{
    ...
    const pN=vid===3?clamp(((cols|0)>>1)||3,2,5):(vid===4?2+(r%3):0), pW=vid===3?.05+s*.02:.04;
    for(let i=1;i<=pN;i++){
      const u=i/(pN+1);
...
export function drawBuilding(c:CanvasRenderingContext2D,x:number,y:number,v:BV,t:number){
