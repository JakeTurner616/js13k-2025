...
export const P = [
  "#fff","#0b0e19","#0a0b13","#121624","#0e111c",
  "#2a2a2f","#232327","#333","#222","#444","#555","#666",
  "#0003","#0004","#eef1fb","#d9dcec","#b7bed2",
  "rgba(210,225,255,.14)","rgba(0,0,0,0)",
  "rgba(40,45,60,.26)","rgba(230,235,255,.10)"
] as const;
export const col = (i:number)=>P[i];

export const gradL=(c:CanvasRenderingContext2D,x0:number,y0:number,x1:number,y1:number,st:[number,number][])=>{
  const g=c.createLinearGradient(x0,y0,x1,y1); for(const [p,i] of st) g.addColorStop(p,col(i)); return g;
};
export const gradR=(c:CanvasRenderingContext2D,x:number,y:number,r:number,st:[number,number][])=>{
  const g=c.createRadialGradient(x,y,0,x,y,POS(r)); for(const [p,i] of st) g.addColorStop(p,col(i)); return g;
};

export const poly=(c:CanvasRenderingContext2D, ci:number, pts:[number,number][])=>{
  c.fillStyle=col(ci); c.beginPath(); c.moveTo(...pts[0]); for(let i=1;i<pts.length;i++) c.lineTo(...pts[i]); c.closePath(); c.fill();
};
...
export const vstrip=(c:CanvasRenderingContext2D,bx:number,_by:number,fw:number,dm:number,u0:number,u1:number,y0:number,h:number,color:number|string,dep:number)=>{
  ...
  c.beginPath(); c.moveTo(x0,y0+d0); c.lineTo(x1,y0+d1); c.lineTo(x1,y0+d1+h); c.lineTo(x0,y0+d0+h); c.closePath(); c.fill();
};
...
export const fbm=(x:number,y:number,s:number)=>{ let a=0,b=1; for(let o=2;o--;) { a+=b*(S(x*.02+s)+S(y*.02+s*1.3)+S((x+y)*.015+s*2.1))/3; b*=.5; x*=1.8; y*=1.8; } return a*.5+.5; };

export const blink=(c:CanvasRenderingContext2D,x:number,y:number,r:number,t:number,a0=.5,ci=0,s=.85,d=.18)=>{
  const p=(t*s)%1, a=p<d?1-(p/d)*.2:a0; c.fillStyle=col(ci); c.globalAlpha=a; c.beginPath(); c.arc(x,y,r,0,7); c.fill(); c.globalAlpha=1;
};
export const antenna=(c:CanvasRenderingContext2D,cx:number,cy:number,h:number,rungs:number,t:number,strokeCi=9,off=0)=>{
  c.strokeStyle=col(strokeCi); c.beginPath(); c.moveTo(cx,cy); c.lineTo(cx,cy-h); c.stroke();
...
export const addB=(d:Draw)=>layersBack.push(d); export const addM=(d:Draw)=>layersMid.push(d); export const addF=(d:Draw)=>layersFront.push(d);
