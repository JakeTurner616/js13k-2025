...
import { P as COLORS, NEON as _NEON, rgba as _rgba } from "../color/Colors";
export const P=COLORS, NEON=_NEON, rgba=_rgba, col=(i:number)=>COLORS[i];
...
export const vstrip=(c:CanvasRenderingContext2D,bx:number,_by:number,fw:number,dm:number,u0:number,u1:number,y0:number,h:number,color:number|string,dep:number)=>{
  const lx0=u0*fw,lx1=u1*fw,x0=bx+(dm>0?lx0*.5:lx0),x1=bx+(dm>0?lx1*.5:lx1),d0=(lx0/fw)*dep*dm,d1=(lx1/fw)*dep*dm;
...
export const hband=(c:CanvasRenderingContext2D,bx:number,by:number,fw:number,dm:number,y0:number,h:number,color:number|string,dep:number)=>vstrip(c,bx,by,fw,dm,0,1,y0,h,color,dep);
