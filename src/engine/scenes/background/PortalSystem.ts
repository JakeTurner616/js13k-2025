...
export class PortalSystem {
  ...
  attachInput(canvas: HTMLCanvasElement, cam: Cam){
    ...
    this.onDown = e=>{
      ...
      const pb=this.player?.body, c=pb?hc(pb):null;
      const cx=c?c.cx:wx, cy=c?c.cy:wy;
  ...
  private cast(
    ...
    for(let tr=0; tr<=MD;){
      if(tX<tY){
        ...
        if(solid(tx,ty)){
          const id=tid(tx,ty), ban=id===FORBID||id===FINISH||id===SPIKE;
  ...
  private spawn(
    ...
    // Orientation: for vertical hits (ax==="x"), face opposite of sX (normal)
  ...
  private teleportIfInside(){
    ...
    // Avoid immediate re-entry loops
    if(this.lastExit){
