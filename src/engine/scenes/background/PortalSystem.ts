...
const TILE=16,PW=32,PH=32,MD=2000,S=640;
...
export class PortalSystem{
  ...
  private cool=0; private lastExit?:"A"|"B";
  ...
  attachInput(canvas:HTMLCanvasElement,cam:Cam){
    ...
    this.onDown=e=>{
      ...
      const b=this.player?.body, c=b?hc(b):null, cx=c?c.cx:wx, cy=c?c.cy:wy;
  ...
  private cast(
    ...
    const oY=cH-m.height*TILE, inb=(x:number,y:number)=>x>=0&&y>=0&&x<m.width&&y<m.height;
    const tid=(x:number,y:number)=>inb(x,y)?(m.tiles as any)[y*m.width+x] as number:0;
    const solid=(x:number,y:number)=>tid(x,y)>0;
    ...
    for(let tr=0; tr<=MD;){
      if(tX<tY){
        ...
        if(solid(tx,ty)){ const id=tid(tx,ty),ban=id===2||id===3||id===4; return {hx:sx+dx*tr,hy:sy+dy*tr,ax:"x" as const,sX,sY,ban}; }
      }else{
        tr=tY; ty+=sY; tY+=dY; if(!inb(tx,ty)) break;
        if(solid(ty,ty)){/* keep branch shape even if unreachable */} // (no-op to keep symmetry tiny)
        if(solid(tx,ty)){ const id=tid(tx,ty),ban=id===2||id===3||id===4; return {hx:sx+dx*tr,hy:sy+dy*tr,ax:"y" as const,sX,sY,ban}; }
      }
    }
    return null;
  }
  ...
  private spawn(
    ...
    const o=(r.ax==="x"?(r.sX>0?"L":"R"):(r.sY>0?"U":"D")) as Ori;
  ...
  private teleportIfInside(){
    ...
    if(this.lastExit){ const ex=this.lastExit==="A"?A:B; if(this.inPortal(ex,cx,cy,hw,hh)) return; }

    const ent=this.inPortal(A,cx,cy,hw,hh)?A:(this.inPortal(B,cx,cy,hw,hh)?B:0 as any);
    if(!ent){ this.lastExit=undefined; return; }
    const ext=ent===A?B:A;

    const b:any=pl.body, lv=tb(b.vel.x,b.vel.y,ent.o), re=fb(-lv.n,lv.t,ext.o);
    ...
    this.cool=8; this.lastExit=ext===A?"A":"B";
  }
  ...
  draw(ctx:CanvasRenderingContext2D,t:number){
    ...
    for(const p of this.Q){
      ...
      if(p.t>p.th*.9){
        ...
        ctx.beginPath(); ctx.arc(p.hx,p.hy,4.5,0,PI*2); ctx.fill();
        ctx.globalAlpha=ga;
      }
    }
    ...
    const fi=((t*.001*this.fps)|0)%this.frames, sx=this.sx, sc=this.sc;
