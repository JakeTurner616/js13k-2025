...
export function zzfxM(n:any,f:any,t:any,e=125):[Float32Array,Float32Array]{
  let l:any,o:number,z:number,r:number,g:any,h:number,x:any,a:any,u:any,c:any,i:any,m:any,d:any,p:any,G:any,M=0,R:any[]=[],
      b:number[]=[],j:number[]=[],k=0,q=0,s=1,v:Record<string,any>={},w=(zzfxR/e*60)>>2;
  for(;s;k++)
    R=[s=a=d=m=0],
    t.map((eIdx:number,dIdx:number)=>{
      for(x=f[eIdx][k]||[0,0,0],s=Number(s)|Number(!!f[eIdx][k]),G=m+(f[eIdx][0].length-2-Number(!a))*w,p=dIdx==t.length-1,o=2,r=m;o<x.length+p;a=++o){
        for(g=x[o],u=((o==x.length+p-1&&p?1:0)||(c!=(x[0]||0)?1:0)|(g?1:0)|0),z=0;z<w&&a;(z++>w-99&&u)?(i+=(i<1?1:0)/99):0)
