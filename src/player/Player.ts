...
export function createPlayer(a:AtlasAnimator, hooks:PlayerHooks = {}){
  ...
  const p = {
    ...
    setAnimation: (name:string)=>{ p.animT0 = performance.now(); }
  };
  ...
  // set by scene after level load (and on resize)
  ...
  function respawn(){
    ...
    b.collide = true; b.grounded = b.cling = b.touchL = b.touchR = false;

    p.touchPortal = false; p.portalContact = 0; p.detach = 0; p.noCling = 0;
  ...
  function update(i:Partial<InputState>, ctx:CanvasRenderingContext2D){
    const inL = !!i.left, inR = !!i.right, inJ = !!i.jump, rel = (!inJ && p.wasJump);

    if (p.portalContact > 0) p.portalContact--;
    p.touchPortal = p.portalContact > 0;

    // collisions toggle inside portal footprint
    ...
    if (p.dead){
      ...
      p.wasJump = inJ; return;
    }

    // bad-aim veto check on release while anchored
    ...
    // ultra-cheap OOB (uses cached bounds + hitbox)
    if (L.on){
    ...
    p.facing = resolveFacing(p.facing>=0?1:-1, p.st===ST.C, anchored, p.aiming, p.clingSide, p.aimAngle, b.vel.x, inL, inR) as 1|-1;
    p.wasJump = inJ;
  }

  function draw(ctx:CanvasRenderingContext2D,t:number){
    const n = AN[p.anim], meta = a.getMeta(n);
    const fps = meta?.fps ?? 6, frames = meta?.frameCount ?? 1;
    const elapsed = Math.max(0, (t - p.animT0) * 1e-3);
    const f = ((elapsed * fps) | 0) % frames;

    const flip = p.facing < 0, dx = flip ? -b.pos.x + b.width * -1 : b.pos.x;
    ctx.save(); if (flip) ctx.scale(-1,1);
    a.drawFrame(ctx, n, f, dx, b.pos.y);
    ctx.restore();

    if (!p.dead && p.aiming && (b.grounded || p.st===ST.C)){
      const px=b.pos.x + b.width*.5, py=b.pos.y + b.height*.5;
      const vx=cos(p.aimAngle)*p.aimPower, vy=-sin(p.aimAngle)*p.aimPower;
      const bad = p.bad || isBadAim(vx,vy,p.clingSide,(p.st===ST.C)||!!(b.touchL||b.touchR));
