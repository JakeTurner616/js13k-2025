...
// 3) Auto-generate src/atlas/tileAtlas.ts in a tiny, pattern-based form (base36 stream)
...
`
  ...
  // --- 4) Compose packed strip image of only used tiles (1 row)
  ...
  // --- 5) Write *tiny* tileAtlas.ts
  // Encode used GIDs as a base36 stream with 2 chars per id (pad left with '0').
  // Example: [11,12,105] -> "0b0c2x"
  const ids36 = used.map(n => n.toString(36).padStart(2, "0")).join("");

  const atlasTs =
`// src/atlas/tileAtlas.ts
// Auto-generated by scripts/compress-map.js (TILE=${TW}, tiles laid in a single row)
// Ultra-compact: GIDs stored as a 2-char base36 stream; build {[gid]:[sx,sy]} once.
export const TILE=${TW};
export const tileAtlasMeta=(()=>{const T=TILE,S="${ids36}";const M={};for(let i=0,j=0;i<S.length;i+=2,j++)M[parseInt(S.slice(i,i+2),36)]=[j*T,0];return M})();\n`;

  fs.mkdirSync(path.dirname(outTileAtlasTs), { recursive: true });
  ...
  console.log(`ℹ Remember to compress ${outPackedPng} for production (and/or embed).`);
}
